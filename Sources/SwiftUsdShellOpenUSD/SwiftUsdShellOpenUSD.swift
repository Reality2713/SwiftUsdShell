...
public final class OpenUSDStageRuntime: Sendable {
    ...
    // MARK: - Variant Authoring
    //
    // Ported from the archived USDTools.USDAdvancedEditing surface. The
    // edit-target distinction (session vs root) that USDTools previously
    // exposed has been intentionally dropped — all edits land on the
    // current edit target (root layer by default) and are exported back to
    // the input URL. Layer-targeting can be re-added when a real consumer
    // requires it.
    ...
    public func createVariantSet(
        ...
        primPath: USDPath,
        setName: USDToken,
        variantNames: [USDToken],
        defaultVariant: USDToken?
    ) throws {
        let stagePtr = UsdStage.Open(std.string(stage.url.path), UsdStage.InitialLoadSet.LoadAll)
        guard stagePtr._isNonnull() else {
            throw SwiftUsdShellError.stageOpenFailed(stage, diagnostic: nil)
        ...
        let usdStage = USDOverlay.Dereference(stagePtr)
        let prim = usdStage.GetPrimAtPath(SdfPath(std.string(primPath.rawValue)))
        guard prim.IsValid() else {
            throw SwiftUsdShellError.primNotFound(stageURL: stage, primPath: primPath)
        }
        var vsets = prim.GetVariantSets()
        let added = vsets.AddVariantSet(std.string(setName.rawValue))
        guard added.IsValid() else {
            throw SwiftUsdShellError.invalidValue(
                "Failed to add variant set '\(setName.rawValue)' on \(primPath.rawValue)"
            )
        }
        var vset = vsets.GetVariantSet(std.string(setName.rawValue))
        for name in variantNames {
            _ = vset.AddVariant(std.string(name.rawValue))
        }
        if let defaultVariant, !defaultVariant.rawValue.isEmpty {
            vset.SetVariantSelection(std.string(defaultVariant.rawValue))
        }
        var exportArgs = pxr.SdfLayer.FileFormatArguments()
        usdStage.Export(std.string(stage.url.path), true, exportArgs)
    }

    public func addVariantReference(
        stage: USDStageURL,
        primPath: USDPath,
        setName: USDToken,
        variantName: USDToken,
        ...
        var vsets = prim.GetVariantSets()
        var vset = vsets.GetVariantSet(std.string(setName.rawValue))
        guard vset.IsValid() else {
            throw SwiftUsdShellError.invalidValue("Variant set not found: '\(setName.rawValue)'")
        }

        _ = USDOverlay.withUsdEditContext(usdStage, usdStage.GetEditTarget()) {
            vset.SetVariantSelection(std.string(variantName.rawValue))
            USDOverlay.withUsdEditContext(vset.GetVariantEditContext(SdfLayerHandle())) {
    ...
    public func addVariantPayload(
        stage: USDStageURL,
        primPath: USDPath,
        setName: USDToken,
        variantName: USDToken,
        assetPath: String,
        payloadPrimPath: USDPath?
    ) throws {
        let stagePtr = UsdStage.Open(std.string(stage.url.path), UsdStage.InitialLoadSet.LoadAll)
        guard stagePtr._isNonnull() else {
            throw SwiftUsdShellError.stageOpenFailed(stage, diagnostic: nil)
        ...
        _ = USDOverlay.withUsdEditContext(usdStage, usdStage.GetEditTarget()) {
            ...
            USDOverlay.withUsdEditContext(vset.GetVariantEditContext(SdfLayerHandle())) {
                ...
                let sdfPath = if let pp = payloadPrimPath?.rawValue, !pp.isEmpty {
                    SdfPath(std.string(pp))
                } else {
                    SdfPath()
                }
                let payload = pxr.SdfPayload(
                    std.string(assetPath),
                    sdfPath,
                    SdfLayerOffset(0.0, 1.0)
                )
                _ = payloads.AddPayload(payload, pxr.UsdListPosition.UsdListPositionBackOfPrependList)
            }
            return true
        }
...
private func normalizedPrimPath(_ rawValue: String) -> String {
...
private extension OpenUSDStageRuntime {
    ...
    public func applyRepairBatch(
        ...
        try withStage(for: request.stageURL, loadPolicy: .loadAll) { sourceStage in
            ...
            guard editStage.Export(std.string(request.outputURL.url.path), false, SdfLayer.FileFormatArguments()) else {
                throw SwiftUsdShellError.invalidValue(
